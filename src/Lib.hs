...
serv countMembers port = withSocketsDo $
			...
			fullLibrary <- getMemberLibrary
			handleMessages bindedSocket fullLibrary
		where
			getMemberLibrary :: [Maybe (Int,String)] -> [Member]	
			getMemberLibrary [Nothing] =  sequence $ foldl \list -> forkIO . getMembersAttributes <*> list $ groupTrackFunc bindedSocket []										
			getMemberLibrary [(Just (id,name)):xs] =  				
				where
				   	groupTrackFunc socket xs = xs : accept socket
					handleMassages sock library = recvFrom sock 150 >>= nextAction library >> handleMassages 
...
removeLib mes@(dataa,nbytes,addrss) [(Member {name, sockAddress, notebook, con, sql}):xs] = if addrss == sockAddress then removeFromHandBook $ notebook $ tail . split " " dataa
				...
				where 
				   removeFromHandBook :: WorkTransactions TypeOfData String -> [String] -> WorkTransactions TypeOfData String																		    	
				   removeFromHandBook wak dat xs [typ:date] = if dat == date then xs else removeFromHandBook
